Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/633ddc0322d7a754f9aff2f31b91e666"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			)
			{
				...
				if (1.0)
				{
					distanceOffset *= lerp(1.0, clamp((distance(_WorldSpaceCameraPos, mul(unity_ObjectToWorld, o.localPos).xyz)), 0.0f, 0.2), 0.1);
				}
				float lineWidth = 0.0;
				#ifdef POI_AUDIOLINK
				...
				if (canGlitch)
				{
					...
					if (AudioLinkIsAvailable() && 0.0)
					{
						...
						if(1.0)
						{
							glitchOffset = glitchAmount * glitchDirection * (1.0 * .01) * audioIntensity;
						} else {
							glitchOffset = glitchAmount * glitchDirection * (1.0 * .01) * step(1.0, randomGlitch);
							glitchOffset += glitchAmount * glitchDirection * (1.0 * .01) * audioIntensity;
						}
					} else {
						glitchOffset = glitchAmount * glitchDirection * (1.0 * .01) * step(1.0, randomGlitch);
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, inout PoiMods poiMods, in PoiLight poiLight)
			{
				...
				Decal0.m_DecalChannelSeparation = 21.0;
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				applyALEmmissionStrength(poiMods, emissionStrength0, float4(0,8,0,0), 3.0, float4(1,5,0,0), 3.0, 1.0);
				applyALCenterOutEmission(poiMods, poiLight.nDotV, emissionStrength0, 1.0, 0.0, float4(0,20,0,0), 1.0, 1.0);
				...
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * 10.0).rgb * lerp(1, poiFragData.baseColor, mixBaseColor).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, 0.0);
				}
				#else
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods, in PoiCam poiCam)
			{
				...
				{
					outlineColor = lerp(outlineColor, float4(0.6231937,0.1721738,0.01459801,1), smoothstep(float4(0,4,0,0).x, float4(0,4,0,0).y, poiMods.audioLink[0.0]));
				}
				#endif
